...
async def get_daa_incident_logs(
    ...
    query = select(DAAIncidentLog).order_by(DAAIncidentLog.created_at.desc()).limit(500)
    ...
    if start_date:
        try:
            ...
            query = query.where(DAAIncidentLog.created_at >= start_dt)
        except ValueError:
            pass
    if end_date:
        try:
            end_dt = datetime.fromisoformat(end_date)
            query = query.where(DAAIncidentLog.created_at <= end_dt)
        except ValueError:
            pass
