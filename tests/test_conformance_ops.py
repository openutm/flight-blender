"""Tests for conformance_monitoring_operations:
...
from conformance_monitoring_operations.operator_conformance_notifications import OperationConformanceNotification
from conformance_monitoring_operations.tasks import check_flight_conformance, check_operation_telemetry_conformance
from conformance_monitoring_operations.utils import FlightBlenderConformanceEngine, is_time_between
...
class TestCheckFlightConformanceTask:
    def test_conformant_calls_telemetry_check(self):
        fd = _make_flight_declaration_for_conformance(state=2, aircraft_id="CORRECT-ID")
        with patch(
            "conformance_monitoring_operations.tasks.FlightBlenderConformanceEngine.check_flight_operational_intent_reference_conformance",
            return_value=1,
        ):
            with patch("conformance_monitoring_operations.tasks.check_operation_telemetry_conformance") as mock_telem:
    ...
    def test_nonconformant_sends_signal(self):
        ...
        with patch(
            ...
            with patch("conformance_monitoring_operations.tasks.custom_signals.flight_operational_intent_reference_non_conformance_signal.send") as mock_signal:
                check_flight_conformance(flight_declaration_id=str(fd.id), session_id="test-sess")
...
class TestCheckOperationTelemetryConformanceTask:
    def test_no_observation_returns_early(self):
        ...
        with patch("conformance_monitoring_operations.tasks.flight_stream_helper.ObservationReadOperations") as mock_obs_cls:
...
class TestOperationConformanceNotification:
    ...
    def test_with_amqp_calls_task(self):
        with patch.dict("os.environ", {"AMQP_URL": "amqp://localhost"}):
            with patch("conformance_monitoring_operations.operator_conformance_notifications.send_operational_update_message") as mock_task:
