...
class SQLAlchemyRIDSyncRepository:
    """Sync repo — used by Celery tasks."""

    def __init__(self, db: Session):
        self.db = db

    def check_subscription_exists_by_subscription_id(self, subscription_id: str) -> bool:
        result = self.db.execute(select(ISASubscriptionORM).where(ISASubscriptionORM.subscription_id == uuid.UUID(subscription_id)))
        return result.scalar_one_or_none() is not None

    def check_subscription_exists_by_view_hash(self, view_hash: int) -> bool:
        result = self.db.execute(select(ISASubscriptionORM).where(ISASubscriptionORM.view_hash == view_hash))
        return result.scalar_one_or_none() is not None

    def get_subscription_by_subscription_id(self, subscription_id: str) -> Optional[ISASubscriptionORM]:
        result = self.db.execute(select(ISASubscriptionORM).where(ISASubscriptionORM.subscription_id == uuid.UUID(subscription_id)))
        return result.scalar_one_or_none()

    def get_subscription_by_id(self, record_id: str) -> Optional[ISASubscriptionORM]:
        return self.db.get(ISASubscriptionORM, uuid.UUID(record_id))
    ...
    def update_subscription_flight_details(self, subscription: ISASubscriptionORM, flights_dict: str) -> bool:
        try:
            subscription.flight_details = flights_dict
            self.db.flush()
            return True
    ...
    def delete_simulated_subscriptions(self) -> bool:
        try:
            self.db.execute(delete(ISASubscriptionORM).where(ISASubscriptionORM.is_simulated == True))  # noqa: E712
            self.db.flush()
            return True
        except Exception:
            return False

    def check_flight_detail_exists(self, flight_detail_id: str) -> bool:
        result = self.db.execute(select(RIDFlightDetailORM).where(RIDFlightDetailORM.id == uuid.UUID(flight_detail_id)))
        return result.scalar_one_or_none() is not None

    def get_flight_detail_by_id(self, flight_detail_id: str) -> Optional[RIDFlightDetailORM]:
        return self.db.get(RIDFlightDetailORM, uuid.UUID(flight_detail_id))

    def create_or_update_flight_detail(self, rid_flight_details_payload) -> Optional[RIDFlightDetailORM]:
        operator_location = _serialize_dataclass(rid_flight_details_payload.operator_location)
        auth_data = _serialize_dataclass(rid_flight_details_payload.auth_data)
        eu_classification = _serialize_dataclass(rid_flight_details_payload.eu_classification)
        uas_id = _serialize_dataclass(rid_flight_details_payload.uas_id)
        detail_id = uuid.UUID(str(rid_flight_details_payload.id))
        existing = self.db.get(RIDFlightDetailORM, detail_id)
        if existing:
            existing.operation_description = rid_flight_details_payload.operation_description
            existing.operator_location = operator_location
            existing.operator_id = rid_flight_details_payload.operator_id
    ...
    def delete_all_flight_details(self) -> bool:
        try:
            self.db.execute(delete(RIDFlightDetailORM))
            self.db.flush()
            return True
        except Exception:
            return False

    def create_operator_rid_notification(self, session_id: str, message: str) -> bool:
        from flight_blender.models.notifications_orm import OperatorRIDNotificationORM

        obj = OperatorRIDNotificationORM(session_id=uuid.UUID(session_id) if session_id else None, message=message)
        self.db.add(obj)
        try:
            self.db.flush()
            return True
